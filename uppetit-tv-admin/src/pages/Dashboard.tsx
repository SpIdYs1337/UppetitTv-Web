...
import { 
  ...
  Layers, Plus, Trash2, CheckCircle2, AlertCircle, Monitor, RefreshCw 
} from 'lucide-react';
...
export const Dashboard = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* 1. ВЕРХНИЕ ВИДЖЕТЫ (МЕТРИКИ СЕТИ) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#141414]/60 backdrop-blur-xl border border-[#2A2A2A]/50 p-5 rounded-2xl flex items-center justify-between shadow-xl">
          ...
          <div className="p-3 bg-[#EA580C]/10 text-[#F97316] rounded-xl border border-[#EA580C]/20">
            <MapPin size={24} />
          </div>
        </div>
      ...
      {/* 2. ПАНЕЛЬ ФИЛЬТРОВ И КНОПОК ДЕЙСТВИЙ */}
      <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 bg-[#141414]/40 p-4 rounded-2xl border border-[#2A2A2A]/50 backdrop-blur-xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            ...
            className="bg-white/10 hover:bg-white/20 text-white font-medium py-2.5 px-4 rounded-xl border border-white/10 transition-colors flex items-center gap-2 text-sm"
          >
            <Plus size={16} /> + Точка
          </button>
          ...
          <button 
            ...
            className="bg-[#EA580C] hover:bg-[#F97316] text-white font-semibold py-2.5 px-5 rounded-xl transition-colors shadow-lg shadow-orange-900/20 border border-[#F97316]/30 flex items-center gap-2 text-sm"
          >
      ...
      {/* 3. ФОРМА БЫСТРОГО ДОБАВЛЕНИЯ ЛОКАЦИИ */}
      {showAddLocForm && (
        <form onSubmit={handleAddLocationSubmit} className="bg-[#141414]/80 border border-[#EA580C]/40 p-5 rounded-2xl backdrop-blur-xl flex flex-col md:flex-row gap-4 items-end">
          <div className="flex-1 space-y-1">
            ...
            <input 
              ...
              placeholder="Например: ТЦ Галерея" 
              className="w-full bg-black/50 border border-[#2A2A2A] rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-[#EA580C]" 
            />
          </div>
          <div className="flex-1 space-y-1">
            ...
            <input 
              ...
              placeholder="Например: Лиговский пр., 30" 
              className="w-full bg-black/50 border border-[#2A2A2A] rounded-xl px-4 py-2.5 text-sm text-white outline-none focus:border-[#EA580C]" 
            />
          </div>
          <button type="submit" className="bg-[#EA580C] text-white font-semibold px-6 py-2.5 rounded-xl hover:bg-[#F97316] transition-colors text-sm">
            Сохранить точку
          </button>
        </form>
      )}

      {/* 4. СПИСОК ГРУППИРОВАННЫХ ТОЧЕК (АККОРДЕОНЫ) */}
          ...
          return (
            <div key={loc.id} className="bg-[#141414]/60 backdrop-blur-xl border border-[#2A2A2A]/50 rounded-2xl overflow-hidden shadow-2xl transition-all">
              ...
              <div 
                ...
                className="p-5 bg-black/30 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 cursor-pointer hover:bg-black/50 transition-colors border-b border-[#2A2A2A]/30"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2.5 bg-[#EA580C]/20 text-[#F97316] rounded-xl">
                    <MapPin size={20} />
                  </div>
                  <div>
                    <div className="flex items-center gap-3">
                      <h3 className="text-xl font-bold text-white">{loc.name}</h3>
                      <span className="text-xs text-gray-400 font-mono bg-black/40 px-2.5 py-1 rounded-md border border-[#2A2A2A]">
                        {loc.address}
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto" onClick={e => e.stopPropagation()}>
                  <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-xl border border-[#2A2A2A]/50">
                    ...
                    <select 
                      onChange={(e) => assignPlaylistToLocation(loc.id, e.target.value || null)}
                    ...
                    >
                      ...
                      {playlists.map(pl => (
                        <option key={pl.id} value={pl.id} className="bg-[#141414]">{pl.name}</option>
                      ))}
                    </select>
                  </div>
              ...
              {!isCollapsed && (
                <div className="p-6">
                  {locDevices.length === 0 ? (
                    <div className="text-center py-8 text-gray-500 text-sm border border-dashed border-[#2A2A2A] rounded-xl">
                      На этой торговой точке пока нет привязанных телевизоров
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                      {locDevices.map((device) => {
                        ...
                        return (
                          <div 
                            key={device.id} 
                            className="bg-[#0A0A0A]/60 border border-[#2A2A2A]/60 rounded-xl p-5 hover:border-[#F97316]/50 transition-all flex flex-col justify-between relative group"
                          >
                            <div>
                              <div className="flex justify-between items-start mb-3">
                                <div>
                                  <h4 className="font-bold text-white flex items-center gap-2">
                                    <Monitor size={16} className="text-[#EA580C]" /> {device.name}
                                  </h4>
                              ...
                              <div className="space-y-1.5 my-4 text-xs text-gray-400">
                                ...
                                <div className="flex justify-between items-center">
                                  ...
                                  <span className="text-white">
                                    {/* ИСПРАВЛЕНИЕ: Красивый вывод округленных гигабайт */}
                                    {typeof device.storageFree === 'number' ? device.storageFree.toFixed(1) : device.storageFree} ГБ свободно
                                  </span>
                                </div>
                                <div className="flex justify-between">
                                  <span>Версия ОС:</span>
                                  {/* ИСПРАВЛЕНИЕ: Красивый вывод версии */}
                                  <span className="text-white">Android {device.androidVersion}</span>
                                </div>
                              </div>

                              {/* ВЫБОР ПЛЕЙЛИСТА ДЛЯ КОНКРЕТНОГО ТВ */}
                              <div className="bg-black/40 rounded-lg p-2.5 border border-[#2A2A2A]/50 mb-2">
                                ...
                                <select 
                                  ...
                                  onChange={(e) => assignPlaylistToDevice(device.id, e.target.value || null)}
                              ...
                              <div className="bg-black/40 rounded-lg p-2.5 border border-[#2A2A2A]/50 mb-2">
                                ...
                                <select 
                                  ...
                                  onChange={async (e) => {
                                    ...
                                    // Отправляем команду на устройство и обновляем локальный стор
                                    const ok = await DeviceService.setRotation(device.shortId, angle);
                                    if (ok) {
                                      updateDeviceRotation(device.id, angle);
                                    }
                                  }}
                                  className="w-full bg-[#141414] border border-[#333] rounded px-2 py-1.5 text-xs text-white outline-none focus:border-[#EA580C]"
                                >
                              ...
                              <button 
                                ...
                                className={`w-full text-xs font-medium py-2 rounded border transition-all flex items-center justify-center gap-2 ${
                                  isOnline 
                                    ? 'bg-white/5 hover:bg-[#EA580C]/20 text-gray-300 hover:text-[#F97316] border-white/5 hover:border-[#EA580C]/30' 
                            ...
                            <div className="mt-5 pt-3 border-t border-[#2A2A2A]/30 flex justify-between items-center text-xs">
                              <span className="text-gray-500">
                                {assignedPl ? `Файлов: ${assignedPl.items.length}` : 'Эфир пуст'}
                              </span>
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation();
